...
export class CategoryViewComponent implements OnInit, OnDestroy {
  ...
  private initFromResolvedData(data: any): void {
    const categoryViewData: CategoryViewRouteData = {
      ...(data[ARTICLE_VIEWS_ROUTE_RESOLVER_DATA_KEYS.CATEGORY_VIEW] ?? null),
    } as CategoryViewRouteData;
    const pageCategoryGroup = {
      ...(categoryViewData?.pageCategoryGroup ?? {}),
    };
    ...
    if (!this.route.firstChild && (!this.category || !this.category.id)) {
      ...
      setTimeout(() =>
        this.ngZone.run(() =>
          this.router.navigate(['/'], {
            queryParams: { categoryId: paramCategoryId },
      ...
      this.categoryArticles = [
        ...(pageCategoryGroup?.pageArticles?.articles ??
          ([] as Array<Article>)),
      ];
      // this.startPage = this.utilsSvc.dateStringToTotalTimeString(pageCategoryGroup?.pageArticles?.previousPage?.updated || '');
      this.endPage = this.utilsSvc.dateStringToTotalTimeString(
        pageCategoryGroup?.pageArticles?.endPage || ''
      );
      this.pageCategoryGroups = [
        ...(categoryViewData?.pageCategoryGroups ??
