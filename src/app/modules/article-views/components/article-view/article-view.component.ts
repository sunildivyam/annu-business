...
import { Article, MetaInfo, MetaService } from '@annubiz/ng-lib';
...
export class ArticleViewComponent implements OnInit {
  ...
  private initFromResolvedData(data: any): void {
    const articleViewData: ArticleViewRouteData = {
      ...(data[ARTICLE_VIEWS_ROUTE_RESOLVER_DATA_KEYS.ARTICLE_VIEW] ?? null),
    };
    ...
    if (!this.route.firstChild && this.article && this.article.id) {
      this.metaService.setPageMeta({
        ...
        title: `${appConfig.metaInfo.title} - ${this.article?.metaInfo?.title}`,
      });
    }
  }
}
